...
with psycopg.connect(conninfo="postgresql://postgres:password@localhost:8001/postgres") as conn:
    with conn.cursor(row_factory=dict_row) as cur:
        cur.execute("select * from wijklabels.woningtypen")
        woningtypen_df = pd.DataFrame.from_records(
            cur.fetchall(), index=["pand_identificatie", "vbo_identificatie"])

_r = woningtypen_df.apply(lambda row: to_woningtype(row["woningtype"]), axis=1)
woningtypen_df["woningtype"] = _r

ep_df = EPLoader("/data/energylabel-ep-online/v20231101_v2_csv.csv").load()

res = woningtypen_df.join(ep_df, how="inner", rsuffix="_ep")

eengezins = res.loc[res["Pand_gebouwsubtype"].isna()]
...
incorrect_pct.plot(kind="pie", autopct='%1.1f%%')
plt.show()
```
